...
//TODO: make sure it's ok with "remove" messages (seems ok when server launched with "--nopersist")
...
//    public NodeJs nodeJs; //?
    public bool useDefaultPort = true;
    public bool useConfigManager = false;
    public string serverAddress = DEFAULT_SERVER_ADDRESS;
    public string serverPort = DEFAULT_SERVER_PORT;
...
////////
        ...
        if (isServer && useDefaultPort) {
            serverPort = DEFAULT_SERVER_PORT;
        }
        if (!isServer && useConfigManager) {
            print("Getting server address and port from Server Configuration Manager");
...
//TODO: check that "serverAddress" & "serverPort" have valid format!
//...
        ...
        if (isAdmin) {
            ...
            if (sbAdmin == null) {
                throw new Exception("SpacebrewClient: SpacebrewAdmin not found in parent object! - Required if 'Is Admin' is set to 'true'");
            }
            print("Admin client");
        ...
        if (isServer) {
            sbServer = gameObject.GetComponentInParent<NodeJs>();
            if (sbServer == null) {
                throw new Exception("SpacebrewClient: NodeJs not found in parent object! - Required if 'Is Server' is set to 'true'");
            }

            print("Starting Spacebrew server");
            sbServer.Init();
            sbServer.Run();
...
//TODO: might have more than 1 element in array?
            sbAdmin.OnRemove(message["remove"][0]["name"], message["remove"][0]["remoteAddress"]);
        }
